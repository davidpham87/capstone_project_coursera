...
This is the reproducible report of the data science cap stone project on 2014.
The code is split into three main parts: 

1. Data preprocessing, with installation of the required packages.
2. Modelling part, where all the transformation and adjustement are made in order to compute the probabilities.
...
* Removeal of punctation (except for the apostroph and the '-');
* Deletion number and all;
* Removal of most badwords;
...
Here is the most important point about the modelling part.
...
* Adjustement probability through Good Turing algorithm and adjusted count;
* Smoothing by back-off, but unnescesary for predition;
* Heavily rely on parallel computing and kill cluster to avoid memory leaks;
* Data.table are used extensively;
* Can not predict anything if the last word was unseen.

The code takes 1 hour to fit the model by computing the n-gram.
...
Then it splits the strings by spaces and find the last two words (or the last two word with a skip words in between).
Then the function returns the 20 most probables following words as a table.
